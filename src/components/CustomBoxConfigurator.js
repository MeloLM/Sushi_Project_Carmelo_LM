...
//   - addCustomBox(payload)      — da aggiungere al CartProvider

const CustomBoxConfigurator = () => {
  const { showToast, addCustomBox } = useCartContext();
  ...
  //
  // Guard doppia: isBoxFull garantisce che currentTotalPieces === maxPieces
  // (nessuna box parziale nel carrello); selectedBox !== null è necessario
  // perché la funzione potrebbe essere richiamata prima che l'utente scelga
  // la taglia (difesa contro edge-case di stato inconsistente).
  //
  // Struttura payload:
  //   id            → UUID generato QUI al momento del click, non in fase di
  //                   dichiarazione dello stato: ogni pressione produce un ID
  //                   distinto, impedendo collisioni tra due box identiche.
  //   name          → stringa leggibile con la capienza scelta.
  //   price         → prezzo fisso della taglia (business rule immutabile).
  //   quantity      → sempre 1: l'utente aggiunge una box alla volta.
  //   isCustomBox   → flag per distinguere nel CartPage i bundle custom
  //                   dagli articoli à-la-carte.
  //   customizations → snapshot dello stato `selections` (solo items con
  //                   qty > 0, già filtrati nel useMemo): verrà serializzato
  //                   su Supabase in una colonna JSONB nella fase successiva.
  ...
  const handleAddToCart = useCallback(() => {
    ...
    const payload = {
      ...
      name:           `Custom Box (${selectedBox.maxPieces} pz)`,
    ...
    addCustomBox(payload);
    setPortions({});
    setStep(1);
    setSelectedBox(null);
    showToast(`"${payload.name}" aggiunta al carrello! 🎉`, 'success');
  }, [isBoxFull, selectedBox, selections, addCustomBox, showToast]);

  // ─── Rendering ───────────────────────────────────────────────────────────

  return (
    <div className="card bg-dark text-white border-warning" aria-label="Custom Box Configurator">
      ...
      <div className="card-header border-warning py-3">
        <div className="d-flex align-items-center justify-content-between">
          <div className="d-flex align-items-center gap-2">
            ...
            <div>
              <h5 className="mb-0">Custom Box Personalizzabile</h5>
...
export default CustomBoxConfigurator;
